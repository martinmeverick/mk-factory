...
0. u `vat_regime = used_goods_margin` ověří nad ZAMČENÝM řádkem a PŘED
   přepočtem: organizace je AKTUÁLNĚ plátce DPH, `margin_vat_rate` je
   podporovaná sazba, měna CZK, každá položka má
   `acquisition_unit_price_minor` (jinak by interní DPH z přirážky byla tiše
   0), ceny nezáporné, množství celé kladné kusy — jinak `InvoiceNotIssuable`
   (číslo řady se nespotřebuje, snapshot loga nevznikne),
...
7. status → issued, issued_at = now, audit `invoice.issued` (včetně
   `vat_regime`).
...
internal_note, currency, vat_regime, margin_vat_rate), položky pouze
description, quantity, unit, unit_price_minor, vat_rate,
acquisition_unit_price_minor a line_* součty (ty stejně přepočítá
kalkulačka; odvozené `margin_*_minor` a `line_margin_*` hodnoty whitelist
NEPŘIJÍMÁ — počítá je výhradně kalkulačka). Klíč mimo whitelist — `organization_id`, `status`,
